...
import { Patient } from "../..";
import { formatDate, calculateAge } from "@/features/utils";
...
const tableHeadersForExistingUsers = {
  name: "Name",
  age: "Age",
  type: "Type",
  diagnosis: "Diagnosis",
...
export const MotherExistingMemberList = ({
  data,
  isLoading,
}: PatientTableProps) => {
    ...
    <div>
      {data && (
        <>
          <Table
            data={data}
            headerMapping={tableHeadersForExistingUsers}
            renderCells={(data) => ({
              name: (
                <div>
                  <div>{data?.name}</div>
              ...
              type: data.last_customer_type ? data.last_customer_type : "-",
              ...
              membership: data.membership_status ? data.membership_status : "-",
                ...
                <div className="flex flex-row gap-5 mx-5">
                  ...
                  >
                    <div
                      ...
                      onClick={() => {
                        navigate(`/mothers/patients/detail/${data._id}`);
                      }}
                    >
                      <FaInfoCircle />
                    </div>
            ...
            footer={
              isLoading && (
                <tr>
                  <td
                    colSpan={Object.keys(tableHeadersForExistingUsers).length}
                  >
