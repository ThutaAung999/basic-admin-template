...
import { getParentSales } from "../api/get-parent-sales";
...
export const MotherFilterExport = ({
  handlePurchasedDatePickerChange,
  handleExpiredDatePickerChange,
  ...
  const { getAllSearchParams } = useParentSaleFilter();

  const { search, start, end, filter_type } = getAllSearchParams();

  const getData = async () => {
    try {
      const { data } = await getParentSales({
        search,
        filter_type,
        start,
        end,
